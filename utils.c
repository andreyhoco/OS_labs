...
int parse(char **arguments, char *input_command) {
	...
	if (strchr(token, '\n') != NULL) {
		if ((arguments[0] = malloc(strlen(token) * sizeof(char))) == NULL) {
			free(input);
			return -1;
		}
		...
		if ((arguments[0] = malloc((strlen(token) + 1) * sizeof(char))) == NULL) {
			free(input);
			return -1;
		}
	...
	while ((token = strtok(NULL, " ")) != NULL) {
		if ((strcmp(token, "") != 0) && (strcmp(token, "\n\0") != 0)) {
			if (strchr(token, '\n') != NULL) {
				if ((arguments[index] = malloc(strlen(token) * sizeof(char))) == NULL) {
					for (int i = 0; i < index; i ++) free(arguments[i]);
					free(input);
					return -1;
				}
